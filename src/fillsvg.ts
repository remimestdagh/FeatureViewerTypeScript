...
class PreComputing {
  ...
  public preComputingLine(object) {
    ...
    this.commons.lineYScale
      .domain([minScore, maxScore])
      .range([0, this.commons.step / 11]);
...
class FillSVG extends ComputingFunctions {
  ...
  public typeIdentifier(feature) {
    ...
    this.commons.yData.push({
      ...
      ladderBorderColor: feature.ladderBorderColor
        ? feature.ladderBorderColor
        : null,
  ...
  public arrow(object, position) {
    ...
    let rectsPro = this.commons.svgContainer
      .append("g")
    ...
    rectsPro
      .selectAll(".line " + object.className)
      .data(dataLine)
      .enter()
      .append("path")
      .attr("y", 0)
    ...
    let rectsProGroup = rectsPro
      .selectAll("." + object.className + "Group")
      .data(object.data)
      .enter()
      .append("g")
      ...
      .style("visibility", (d) => {
        if (d.label) {
          return this.commons.scaling(d.y) - this.commons.scaling(d.x) >
            d.label.length * 8 && object.height > 11
            ? "visible"
            : "hidden";
        } else return "hidden";
      })
    ...
    this.commons.YPosition +=
      this.commons.level < 2
        ? uniqueShift
        : (this.commons.level - 1) * rectShift + uniqueShift;
  }
  ...
  public tagArea(object, thisYPosition) {
    ...
    let featureArea = this.commons.tagsContainer
      .append("g")
      .attr("class", "tagGroup")
      .attr("id", id)
      ...
      for (const bt of object.sidebar) {
        if (bt.type) {
          if (
            ...
            this.commons.logger.error("Unknown type of button", {
              method: "addFeatures",
              fvId: this.commons.divId,
              featureId: object.id,
              buttonId: bt.buttonId,
            });
          } else {
            ...
            if (bt.type == "button") {
              ...
              let color =
                "rgba(" + [colorrgb.r, colorrgb.g, colorrgb.b].join(",") + ")";
              let textColor = this.isLight(color) ? "black" : "white";
              content = `<button id="${
                bt.id
              }" class="mybutton" style="background-color: ${color.replace(
                ")",
                ",0.8)"
              )}; color: ${textColor}; border: 2px solid ${color}"><span style="font-size: small">${disordersString}</span></button>`;
          ...
          try {
            let contentwidth = 0;
            if (bt.width) contentwidth = bt.width;
            else
              contentwidth = (<HTMLElement>(
                d3.select(`#${bt.id}`).select("*").node()
          ...
          this.commons.logger.error(
            "Neither html content nor type of button is specified",
            {
              method: "addFeatures",
              fvId: this.commons.divId,
              featureId: object.id,
  ...
  public sequence(seq, start = 0) {
    ...
    let sequenceAAs = this.commons.svgContainer
      .append("g")
    ...
    sequenceAAs
      ...
      .attr("x", (d, i) => {
        // index starts from 0
        return this.commons.scaling.range([
          2,
          this.commons.viewerOptions.width - 2,
        ])(i + start);
      })
  ...
  public sequenceLine() {
    ...
    if (this.commons.viewerOptions.dottedSequence) {
      let dottedSeqLine = this.commons.svgContainer
        .selectAll(".sequenceLine")
        .data([
          [
            { x: 1, y: this.commons.step - this.commons.elementHeight / 2 },
            {
              x: this.commons.fvLength,
      ...
      dottedSeqLine.transition().duration(500).style("stroke-opacity", 1);
    }
  }
  ...
  public unique(object, position) {
    ...
    rectsPro
      .selectAll("." + object.className + "Unique")
      .data(readyData)
      .enter()
      .append("rect")
      // .attr("clip-path", "url(#clip)") // firefox compatibility
      ...
      .attr("width", (d) => {
        if (
          this.commons.scaling(d.x + 0.4) - this.commons.scaling(d.x - 0.4) <
          2
        )
          return 2;
        else
  ...
  public lollipop(object, position) {
    let circlesPro = this.commons.svgContainer
      .append("g")
    ...
    // lollipop base
    circlesPro
      .selectAll("." + object.className + "Lollipop")
      .data(readyData)
      .enter()
      .append("line")
      .attr("x1", (d) => {
    ...
    circlesPro
      ...
      .attr("r", (d) => {
        if (d.y <= 1) {
          ...
          this.commons.logger.warn("Maximum circle radius is 1", {
            method: "addFeatures",
            fvId: this.commons.divId,
            featureId: object.id,
          });
  ...
  public circle(object, position) {
    ...
    circlesPro
      .selectAll("." + object.className + "Circle")
      .data(readyData)
      .enter()
      .append("circle")
      //.attr("clip-path", "url(#clip)")
  ...
  public path(object, position) {
    ...
    let pathsDB = this.commons.svgContainer
      .append("g")
    ...
    pathsDB
      .selectAll(".path" + object.className)
      .data(object.data)
      .enter()
      .append("path")
      //.attr("clip-path", "url(#clip)") // firefox compatibility
  ...
  public fillSVGLine(object, position = 0) {
    ...
    let histoG = this.commons.svgContainer
      .append("g")
    ...
    object.data.forEach((dd, i) => {
      histoG
        ...
        .attr(
          "class",
          "element " + object.className + " " + object.className + i
        )
        // d3 v4
        .attr(
          "d",
          this.commons.lineGen.y((d) => {
  ...
  public multipleRect(object, position = 0, level = this.commons.level) {
    ...
    let rects = this.commons.svgContainer
      .append("g")
  ...
  public reset_axis() {
    ...
    this.commons.svgContainer.select(".x.axis").call(this.commons.xAxis);
  }
