...
class FeatureViewer {
  ...
  private parseUserOptions(options: UserOptions): void {
    ...
    // if zoom specified
    this.commons.viewerOptions.offset = {
      start: 0,
      end: this.commons.fvLength + 1,
    };
    if (options && options.offset) {
      this.commons.viewerOptions.offset = options.offset;
      if (options.offset.start < 1) {
        this.commons.viewerOptions.offset.start = 1;
        this.commons.logger.warn(
          "Offset.start should be > 0. Thus, it has been reset to 1.",
          { fvId: this.divId }
        );
      }
    }
    ...
    // brush active
    this.commons.viewerOptions.brushActive = options.brushActive
      ? options.brushActive
      : true;
    ...
    if (options && options.sideBar) {
      if (typeof options.sideBar === "string") {
        this.commons.viewerOptions.tagsTrackWidth = Number(
          options.sideBar.match(/[0-9]+/g)[0]
        );
        ...
        this.commons.viewerOptions.tagsTrackWidth = 100;
        this.commons.logger.warn(
          `Automatically set tagsTrackWidth to ${this.commons.viewerOptions.tagsTrackWidth}`,
          { fvId: this.divId }
        );
      }
    }
    this.commons.viewerOptions.backup.tagsTrackWidth =
      this.commons.viewerOptions.tagsTrackWidth;

    // set width of flags
    this.commons.viewerOptions.labelTrackWidth = 200;
    if (options && options.flagTrack) {
      if (typeof options.flagTrack === "string") {
        this.commons.viewerOptions.labelTrackWidth = Number(
          options.flagTrack.match(/[0-9]+/g)[0]
        );
    ...
    this.commons.viewerOptions.backup.labelTrackWidth =
    ...
    if (options && options.flagTrackMobile) {
      if (typeof options.flagTrackMobile === "string") {
        this.commons.viewerOptions.labelTrackWidthMobile = Number(
          options.flagTrackMobile.match(/[0-9]+/g)[0]
        );
      } else if (typeof options.flagTrackMobile === "number") {
        this.commons.viewerOptions.labelTrackWidthMobile =
          options.flagTrackMobile;
      } else if (typeof options.flagTrackMobile === "boolean") {
        this.commons.viewerOptions.labelTrackWidthMobile =
          options.flagTrackMobile ? 30 : 0;
      } else {
        this.commons.viewerOptions.labelTrackWidthMobile = 30;
        this.commons.logger.warn(
          `Automatically set tagsTrackWidth to ${this.commons.viewerOptions.tagsTrackWidth}`,
    ...
    if (myd3node) {
      ...
      if (totalwidth < this.commons.mobilesize) {
        ...
        this.commons.viewerOptions.margin.left =
          this.commons.viewerOptions.labelTrackWidthMobile;
    ...
    if (myvod3node) {
      this.commons.viewerOptions.width = (<HTMLElement>(
        myvod3node
      )).getBoundingClientRect().width;
      this.commons.viewerOptions.height =
        600 -
        this.commons.viewerOptions.margin.top -
        this.commons.viewerOptions.margin.bottom;
    }
    // backgroundcolor
    this.commons.backgroundcolor = options.backgroundcolor
  ...
  private addYAxis() {
    // flags box
    this.commons.yAxisSVG = this.commons.svg
      .append("g")
      .attr("class", "pro axis")
      .attr(
        "transform",
        "translate(0," + this.commons.viewerOptions.margin.top + ")"
      );
    this.commons.yAxisSVG
      .append("rect")
  ...
  private updateYAxis() {
    ...
    this.commons.yAxisSVGGroup
      ...
      .attr("transform", (d) => {
        let y = 0;
        if (d.id == "mycurve" || d.id == "useUniqueId") {
          y = 5;
        }
    ...
    this.commons.yAxisSVGGroup
      ...
      .attr("width", (d) => {
        ...
        if (this.commons.viewerOptions.mobileMode) {
          ...
          let margin =
            20 +
            this.commons.viewerOptions.ladderSpacing *
              this.commons.viewerOptions.maxDepth; // 20 + (20 * d.flagLevel) --> 0
      ...
      .html(([i, d]) => {
        if (d.ladderLabel) {
          return d.id !== "fv_sequence" && i === d.flagLevel - 1
            ? `<div class="badge-feature badge-feature-sm" style="background-color: ${
                d.ladderBgColor
              }; color: ${d.ladderColor};
                        border-style: ${
                          d.hasSubFeatures ? "solid" : "hidden"
    ...
    ladderGroup
      .attr("transform", ([i, d]) => {
        if (d.ladderLabel) {
          const margin =
            this.commons.viewerOptions.margin.left +
            (i - 1) * this.commons.viewerOptions.ladderSpacing;
          const ladderWidth =
            this.commons.viewerOptions.ladderSpacing *
            this.commons.viewerOptions.maxDepth;
      ...
      .attr("width", this.commons.viewerOptions.ladderWidth)
      .attr("height", this.commons.viewerOptions.ladderHeight)
      .attr("fill", ([i, d]) => {
        return d.id !== "fv_sequence" && i === d.flagLevel - 1
          ? d.ladderColor
          : "rgba(255, 255, 255, 0)";
    ...
    ladderGroup.append("text").attr("transform", ([i, d]) => {
      if (d.ladderLabel) {
        const margin =
          this.commons.viewerOptions.margin.left +
          (i - 1) * this.commons.viewerOptions.ladderSpacing;
        const ladderWidth =
          ...
          this.commons.viewerOptions.maxDepth;
        const x =
          margin - ladderWidth + this.commons.viewerOptions.ladderWidth / 2;
  ...
  private brushend() {
    ...
    if (this.commons.featureSelected) {
      d3.select(`#${this.divId}`)
        .select(`#${this.commons.featureSelected}`)
        .style("fill-opacity", "0.6");
    ...
    if (
      currentEvent.sourceEvent !== null &&
      typeof currentEvent.sourceEvent.target !== "function"
    ) {
      ...
      if (this.commons.extent) {
        ...
        let borders = [this.commons.extent[0], this.commons.extent[1]].map(
          this.commons.scaling.invert,
        ...
        if (extentLength > this.commons.viewerOptions.zoomMax) {
          ...
          let currentShift = start
            ? start
            : this.commons.viewerOptions.offset.start;
          ...
          if (this.commons.viewerOptions.showSequence) {
            if (seqCheck === false) {
              ...
              this.fillSVG.sequence(
                this.sequence.substring(start, end),
                this.commons.seqShift
              );
            }
          }
        } else {
          ...
          this.commons.logger.warn(
            "Zoom greater than " +
              this.commons.viewerOptions.zoomMax +
              " is prevented",
            { fvId: this.divId }
          );
        }
      ...
      d3.select(`#${this.divId}`)
        .select(".brush")
        .call(this.commons.brush.move, null);
    } else {
      return;
    }
  }
  ...
  private resizeForMobile() {
    ...
    if (this.commons.viewerOptions.mobileMode) {
      d3.select(`#${this.divId}`)
        .selectAll("#ladder-group")
        .attr("transform", ([i, d]) => {
          return (
            "translate(" +
            (d.flagLevel - 1) * this.commons.viewerOptions.ladderSpacing +
            "," +
            (d["y"] + 3.14) +
            ")"
          );
      ...
      d3.select(`#${this.divId}`)
        ...
        .attr("transform", ([i, d]) => {
          ...
          return (
            ...
            (this.commons.viewerOptions.labelTrackWidth -
              margin +
              d.flagLevel * this.commons.viewerOptions.ladderSpacing) +
            "," +
            (d["y"] + 3.14) +
            ")"
          );
    ...
    this.commons.svgContainer.attr(
      "transform",
      "translate(" + this.commons.viewerOptions.margin.left.toString() + ",12)"
    );
  ...
  private calcFlagWidth(d) {
    ...
    let space =
      this.commons.viewerOptions.labelTrackWidthMobile - 15 - totalspace;
    if (space < 20) {
      return "0px";
    } else {
      return space + "px";
  ...
  private updateWindow() {
    ...
    if (d3.select(`#${this.divId}`).node() !== null) {
      ...
      this.commons.viewerOptions.width =
        totalwidth -
        this.commons.viewerOptions.margin.left -
        this.commons.viewerOptions.margin.right -
        17;

      // resize containers
      this.commons.svg.attr("width", totalwidth);
      this.commons.svg
        .select("clipPath rect")
        .attr("width", this.commons.viewerOptions.width);
      if (this.commons.viewerOptions.brushActive) {
        d3.select(`${this.commons.divId} .background`).attr(
          "width",
          this.commons.viewerOptions.width - 10
        );
      }

      // resize brush
      d3.select(`#${this.commons.divId}`)
        .select(".fvbrush")
        .call(this.commons.brush.move, null);

      // new scaling
      this.commons.scaling.range([2, this.commons.viewerOptions.width - 2]);
      this.commons.scalingPosition.domain([
        0,
        this.commons.viewerOptions.width,
      ]);
        ...
        if (this.getCurrentZoom() == undefined || seq === false) {
          ...
          this.fillSVG.sequence(
            this.sequence.substring(
              this.commons.viewerOptions.offset.start,
              this.commons.viewerOptions.offset.end
            ),
            this.commons.viewerOptions.offset.start
          );
        }
      ...
      this.transition_data(
        ...
        this.commons.current_extend.start
      );
      this.fillSVG.reset_axis();
  ...
  private init(div) {
    ...
    this.commons.scalingPosition = d3
      .scaleLinear()
    ...
    d3.select(`#${this.divId}`).append("foreignObject").html(overlayhtml);

    this.commons.lineBond = d3
      .line()
    ...
    this.commons.lineYScale = d3.scaleLinear().domain([0, -30]).range([0, -20]);

    this.commons.line = d3
      .line()
    ...
    let tickArray = Array.from(Array(this.commons.fvLength).keys()).filter(
    ...
    //Create Axis
    this.commons.xAxis = d3
      .axisBottom(this.commons.scaling)
    ...
    this.commons.right_chevron =
      "M12.95 10.707l0.707-0.707-5.657-5.657-1.414 1.414 4.242 4.243-4.242 4.243 1.414 1.414 4.95-4.95z";
    this.commons.down_chevron =
      "M9.293 12.95l0.707 0.707 5.657-5.657-1.414-1.414-4.243 4.242-4.243-4.242-1.414 1.414z";

    // Define the divs for the tooltip
    this.commons.tooltipDiv = d3
      .select(`#${this.divId}`)
      .append("div")
      ...
      .style("opacity", 0)
      .style("z-index", 1070);
    this.commons.customTooltipDiv = d3
      .select(`#${this.divId}`)
      .append("div")
    ...
    if (this.commons.viewerOptions.toolbar) {
      let headerOptions = document.querySelector(div + " .svgHeader")
        ? d3.select(div + " .svgHeader")
        : d3.select(div).append("div").attr("class", "svgHeader");
      ...
      if (this.commons.viewerOptions.toolbarPosition) {
        ...
        headerOptions.attr(
          "style",
          "color: rgba(39, 37, 37, 0.71); display: flex; justify-content: " +
            this.commons.viewerOptions.toolbarPosition
        );
      } else {
      ...
      if (!document.querySelector(div + " .header-position")) {
        ...
        button
          ...
          .attr(
            "d",
            "M10 20s-7-9.13-7-13c0-3.866 3.134-7 7-7s7 3.134 7 7v0c0 3.87-7 13-7 13zM10 9c1.105 0 2-0.895 2-2s-0.895-2-2-2v0c-1.105 0-2 0.895-2 2s0.895 2 2 2v0z"
          );
        button.append("text").text("Position:");
        headerPosition
          .append("div")
          .style("display", "inline-block")
      ...
      if (!document.querySelector(div + " .header-help")) {
        ...
        headerHelp
          ...
          .attr(
            "d",
            "M2.93 17.070c-1.884-1.821-3.053-4.37-3.053-7.193 0-5.523 4.477-10 10-10 2.823 0 5.372 1.169 7.19 3.050l0.003 0.003c1.737 1.796 2.807 4.247 2.807 6.947 0 5.523-4.477 10-10 10-2.7 0-5.151-1.070-6.95-2.81l0.003 0.003zM9 11v4h2v-6h-2v2zM9 5v2h2v-2h-2z"
          );
      }
    }
      ...
      .on("contextmenu", (d, i) => {
        ...
        if (CustomEvent) {
          let event = new CustomEvent(
            this.commons.events.CLEAR_SELECTION_EVENT,
            { detail: {} }
          );
          this.commons.svgElement.dispatchEvent(event);
        } else {
          this.commons.logger.warn("CustomEvent is not defined", {
            fvId: this.divId,
          });
        }
    ...
    // tags space
    this.commons.tagsContainer = this.commons.svg
      .append("g")
      .attr(
        "transform",
        "translate(" +
    ...
    if (this.commons.viewerOptions.showSequence) {
      if (
        this.calculate.displaySequence(
          this.commons.viewerOptions.offset.end -
            this.commons.viewerOptions.offset.start
        )
      ) {
  ...
  private addFeatureCore(object, flagLevel = 1, position = null) {
    ...
    if (this.commons.animation) {
      if (CustomEvent) {
        let event = new CustomEvent(
          this.commons.events.ANIMATIONS_FALSE_EVENT,
          { detail: {} }
        );
  ...
  private drawFeatures() {
    ...
    if (this.commons.features.length > 100) {
      ...
      this.commons.logger.warn(
        "Animation is turned off with more than 100 features",
        {
          method: "addFeatureCore",
          fvId: this.divId,
          featuresNumber: this.commons.features.length,
        }
      );
    }
  ...
  public resetHighlight(resetLastHighlight = true) {
    ...
    if (CustomEvent) {
      let event = new CustomEvent(this.commons.events.CLEAR_SELECTION_EVENT, {
        detail: {},
      });
      this.commons.svgElement.dispatchEvent(event);
    } else {
  ...
  public resetZoom() {
    ...
    this.commons.scaling.domain([
      this.commons.viewerOptions.offset.start,
      this.commons.viewerOptions.offset.end,
    ]);
    this.commons.scalingPosition.range([
      this.commons.viewerOptions.offset.start,
      this.commons.viewerOptions.offset.end,
    ]);
    let seq = this.calculate.displaySequence(
      this.commons.viewerOptions.offset.end -
        this.commons.viewerOptions.offset.start
    );
    ...
    // Fire Event
    if (CustomEvent) {
      this.commons.svgElement.dispatchEvent(
        new CustomEvent(this.commons.events.ZOOM_EVENT, {
          detail: {
            start: 1,
      ...
      if (CustomEvent) {
        let eventDetail = { detail: flag_detail_object },
          event = new CustomEvent(
            this.commons.events.FLAG_SELECTED_EVENT,
            eventDetail
          );
        this.commons.svgElement.dispatchEvent(event);
        ...
        if (this.commons.viewerOptions.showSubFeatures && d.hasSubFeatures) {
          ...
          for (i = 0; result == null && i < this.commons.features.length; i++) {
            result = this.calculate.searchTree(
              this.commons.features[i],
              flag_detail_object.id
            );
          }
          ...
          if (featureToChange) {
            ...
            this.commons.logger.warn("Feature not found in feature array", {
              fvId: this.commons.divId,
              featureId: flag_detail_object.id,
            });
          }
        }
      } else {
  ...
  public highlightPosition(region, reset = true) {
    ...
    let start = this.commons.scaling(region.start - 0.5);
    let end = this.commons.scaling(region.end + 0.5);
    // remove selection rectangle if already there
    let selectRect;
    // color the background
    if (this.commons.svgContainer.node()) {
      let currentContainer = this.commons.svgContainer
        .node()
  ...
  public onRegionSelected(listener) {
    this.commons.svgElement.addEventListener(
      this.commons.events.FEATURE_SELECTED_EVENT,
      listener
    );
  }
  ...
  public onButtonSelected(listener) {
    this.commons.svgElement.addEventListener(
      this.commons.events.TAG_SELECTED_EVENT,
      listener
    );
  }
  ...
  public onZoom(listener) {
    this.commons.svgElement.addEventListener(
      this.commons.events.ZOOM_EVENT,
      listener
    );
  }
  ...
  public addFeatures(featureList: FeaturesList) {
    ...
    if (featureids.length !== featureList.length) {
      this.commons.logger.error(
        "Feature ids are not present on all features, creating random ids",
    ...
    for (let f of featureList) {
      if (!regexIds.test(f.id)) {
        this.commons.logger.error(
          "Id " + f.id + "is not a valid html id, substituting it randomly",
          { method: "addFeatures", fvId: this.divId }
        );
        f.id = "customid" + Math.random().toString(36).substring(7);
      }
    ...
    if (uniqueIds.length !== featureList.length) {
      this.commons.logger.error(
        "Feature ids are not unique, substituting the non unique ones randomly",
        { method: "addFeatures", fvId: this.divId }
      );
      let idsencountered = [];
      for (let f of featureList) {
    ...
    if (!this.commons.viewerOptions.maxDepth) {
      this.commons.viewerOptions.maxDepth = Math.max(
        ...this.commons.features.map((f) => this.getLevel(f, 1))
      );
    }
    ...
    if (unprocessedIds.length !== 0) {
      this.commons.logger.error("Subfeatures with no known parentId", {
        method: "addFeatures",
        fvId: this.divId,
        features: unprocessedIds,
      });
    }
