...
export class SubfeaturesTransition extends ComputingFunctions {
  ...
  public position(gElement, parentElementRow) {
    gElement.attr("position", "element(#" + parentElementRow + ")");
  }
...
export class Transition extends ComputingFunctions {
  public basalLine(object) {
    let container = this.commons.svgContainer.select(
      `#c${object.id}_container`
    );
    container.selectAll(".line").attr("d", this.commons.line);
  }
  ...
  public rectangle(object) {
    ...
    if (this.commons.animation) {
      transit1.transition().duration(500);
      transit2.transition().duration(500);
    }
    // transition
    transit1.attr("transform", (d) => {
      return "translate(" + this.rectX(d) + ",0)";
    });
    transit2.attr("width", this.rectWidth2);

    // transition to text
    container
      .selectAll("." + object.className + "Text")
      .attr("transform", (d) => {
  ...
  public unique(object) {
    ...
    let transit;
    if (this.commons.animation) {
      transit = container.selectAll(".element").transition().duration(500);
    } else {
  ...
  public lollipop(object) {
    ...
    let transit1, transit2;
    if (this.commons.animation) {
      transit1 = container.selectAll(".element").transition().duration(500);
      transit2 = container.selectAll(".lineElement").transition().duration(500);
    } else {
  ...
  public path(object) {
    ...
    container.selectAll(".line").attr(
      "d",
      this.commons.lineBond
        .x((d) => {
          return this.commons.scaling(d["x"]);
        })
        .y((d) => {
  ...
  public arrow(object) {
    ...
    container
      ...
      .attr("transform", (d) => {
        let offset =
          d.direction === "left" ? this.commons.elementHeight / 2 : 0;
        if (d.label && this.commons.scaling(d["x"]) < 0) {
  ...
  public lineTransition(object) {
    ...
    container.selectAll(".line " + object.className).attr("d", (d) => {
